...
// Returns current time adjusted to server clock
...
async function loadSiteConfig() {
    try {
        ...
        const t1 = Date.now();
        if (siteConfig.server_time) {
            const rtt = t1 - t0;
            serverTimeOffset = new Date(siteConfig.server_time).getTime() + Math.round(rtt / 2) - t1;
        }
    ...
    if (ctfStart && now < ctfStart) {
        ...
        if (customOpening.trim()) {
            ...
            if (openingTimerVisible) {
                ...
                // Even if timer is hidden, reload when CTF starts
                const checkStart = setInterval(() => {
                    if (getServerNow() >= ctfStart) { clearInterval(checkStart); location.reload(); }
                }, 1000);
            }
        }
        return;
...
async function renderChallenges(container) {
    ...
    if (ctfStart && now < ctfStart && !isAdmin) {
        ...
        `;
        // Auto-reload when CTF starts
        const waitForStart = setInterval(() => {
            if (getServerNow() >= ctfStart) { clearInterval(waitForStart); location.reload(); }
        }, 1000);
        return;
    }
            ...
            <div class="container">
                ...
                ${data.frozen ? `
                    <div class="frozen-indicator" style="background: linear-gradient(135deg, rgba(100,149,237,0.2), rgba(65,105,225,0.1)); border: 1px solid #6495ed; padding: 1rem; border-radius: 12px; text-align: center; margin-bottom: 1.5rem;">
                        ...
                        <span style="color: #6495ed; font-weight: 600; margin-left: 0.5rem;">Scoreboard is frozen${data.freeze_to ? ' until ' + formatDate(data.freeze_to) : ''}</span>
                    </div>
                ` : ''}
        ...
        <div class="card mb-4">
            ...
            <form id="start-time-form">
                ...
                <div style="display: flex; gap: 0.5rem; margin-bottom: 1rem; flex-wrap: wrap;">
                    ...
                    <button type="button" class="btn btn-ghost" onclick="setOpeningQuickTime('30s')">+30 Sec</button>
                    <button type="button" class="btn btn-ghost" onclick="setOpeningQuickTime('1m')">+1 Min</button>
...
async function clearScoreFreeze() {
    try {
        await api('/admin/config/bulk', {
            method: 'PUT',
            body: { settings: { score_freeze_from: '', score_freeze_to: '', score_freeze_time: '' } }
        });
        await loadSiteConfig();
        showToast('Score freeze cleared!', 'success');
        loadAdminSection('running-timer');
    } catch (err) {
        showToast(err.message, 'error');
    }
        ...
        await api('/admin/config/bulk', {
            ...
            body: {
                settings: {
                    ...
                    score_freeze_from: form.score_freeze_time?.value ? new Date(form.score_freeze_time.value).toISOString() : '',
